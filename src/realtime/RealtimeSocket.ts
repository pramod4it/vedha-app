...
import { WEBSOCKET_MAX_RETRIES, WEBSOCKET_RECONNECT_DELAY } from './constants';
...
export class RealtimeSocket {
  ...
  constructor(config: RealtimeSocketConfig, callbacks: RealtimeSocketCallbacks) {
    this.config = {
      autoReconnect: true,
    ...
    try {
      ...
      this.ws.onmessage = (event) => {
        if (typeof event.data === 'string') {
          const transcript = this.extractTranscript(event.data);

          if (transcript) {
            console.info('[RealtimeSocket] Backend transcript received length=', transcript.length);
      ...
      this.ws.onclose = (event) => {
        console.info('[RealtimeSocket] Backend manual websocket closed', event.code, event.reason);
        this.connected = false;
        this.ws = null;
  ...
  private scheduleReconnect(lastError?: unknown) {
    ...
    const maxAttempts = this.config.maxReconnectAttempts || WEBSOCKET_MAX_RETRIES;

    if (this.reconnectAttempts >= maxAttempts) {
      console.error('[RealtimeSocket] Backend manual websocket reconnect failed');
      this.callbacks.onReconnectFailed?.();
      this.callbacks.onClose?.();
      this.pendingConnectReject?.(lastError || new Error('Backend websocket reconnect failed'));
      this.pendingConnectResolve = null;
      this.pendingConnectReject = null;
      return;
    }

    const delayMs = this.config.reconnectDelayMs || WEBSOCKET_RECONNECT_DELAY;
  ...
  private extractTranscript(payload: string): string {
    try {
      const parsed = JSON.parse(payload) as Record<string, unknown>;

      if (parsed.type !== 'transcriptUpdate') {
        return '';
      }
