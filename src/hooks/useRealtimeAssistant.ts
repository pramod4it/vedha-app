...
let activeManualSocket: RealtimeSocket | null = null;
let activeManualSocketOwner = 0;
let nextManualSocketOwner = 1;
...
async function ensureSocketMetadata(
  ...
  const now = Date.now();
  const existingStartedAt = metadata?.chatSessionStartedAt || 0;
  const isExpired =
  ...
  const nextMetadata: InterviewMetadata = {
    ...
    chatSessionId: isExpired ? createChatSessionId() : metadata.chatSessionId,
    chatSessionStartedAt: isExpired ? now : existingStartedAt,
    chatContextClearedAt: isExpired ? undefined : metadata?.chatContextClearedAt,
...
function metadataForSocket(metadata: InterviewMetadata): InterviewMetadata {
  return {
    ...metadata,
    resumeSummary: limitText(metadata.resumeSummary, LIVE_RESUME_CONTEXT_LIMIT),
  };
}
...
export function useRealtimeAssistant(
  ...
  const [status, setStatus] = useState<AssistantStatus>(ASSISTANT_STATUS.IDLE);
  ...
  if (ownerRef.current === 0) {
    ownerRef.current = nextManualSocketOwner;
    nextManualSocketOwner += 1;
  }

  const start = useCallback(async () => {
    if (socketRef.current?.isConnected() || startingRef.current) {
      return;
    }
    ...
    try {
      ...
      if (!mountedRef.current || startSequenceRef.current !== startSequence) {
        return;
      }

      const socketMetadata = await ensureSocketMetadata(
      ...
      const answerDepth = socketMetadata.answerDepth;

      activeManualSocket?.disconnect();
      activeManualSocket = null;
        ...
        },
        {
          onOpen: () => {
            ...
            socketRef.current?.sendControlMessage({
              ...
              value: metadataForSocket(socketMetadata),
            });
          ...
          onReconnectFailed: () => {
            ...
            setError('Backend connection lost. Please restart the app.');
      ...
      activeManualSocket = socket;
      activeManualSocketOwner = ownerRef.current;
  ...
  const submitManualQuestion = useCallback((question: string): boolean => {
    ...
    if (!socketRef.current?.isConnected()) {
      setError('Backend connection is not ready. Please wait a moment and try again.');
      return false;
    }
  ...
  const resetChatSession = useCallback(async () => {
    try {
      ...
      const socketMetadata = await ensureSocketMetadata(existingMetadata, config.solutionLanguage);
      const clearedAt = Date.now();

      const nextMetadata: InterviewMetadata = {
        companyName: socketMetadata.companyName || '',
        interviewerName: socketMetadata.interviewerName || '',
        answerDepth: socketMetadata.answerDepth || 'short',
        ...
        solutionLanguage:
          config.solutionLanguage || socketMetadata.solutionLanguage || ProgrammingLanguage.Java,
        resumeSummary: socketMetadata.resumeSummary || '',
      };
  ...
  useEffect(() => {
    ...
    window.addEventListener('vedha-answer-depth-changed', handleAnswerDepthChange);

    return () => {
      window.removeEventListener('vedha-answer-depth-changed', handleAnswerDepthChange);
    };
  }, []);
